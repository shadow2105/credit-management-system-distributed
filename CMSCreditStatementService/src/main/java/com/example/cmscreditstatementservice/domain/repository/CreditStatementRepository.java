...
import com.example.cmscreditstatementservice.domain.CreditAccount_;
import com.example.cmscreditstatementservice.domain.CreditStatement;
import com.example.cmscreditstatementservice.domain.CreditStatement_;
...
public interface CreditStatementRepository extends JpaRepository<CreditStatement, UUID>,
        JpaSpecificationExecutor<CreditStatement> {
    /*
     * view all credit statements by customerId ordered by creditStatementDate
     *
      SELECT s.credit_statement_id, s.credit_statement_datetime, s.due_datetime, s.amount_due, s.minimum_payment,
      s.credit_account_id, c.customer_id
      FROM credit_statements s
      JOIN credit_accounts c
      ON s.credit_account_id = c.id
      WHERE c.customer_id = ?
      ORDER BY s.credit_statement_date;

     * We didn't explicitly specify the join column because Spring Data JPA automatically infers the join column based
     * on the relationship between entities. In the CreditStatement entity, the @ManyToOne annotation is used to define
     * the relationship with the CreditAccount entity. By default, Spring Data JPA will look for a foreign key column
     * (@JoinColumn) in the CreditStatement table that references a column of the CreditAccount table (id).
     * It infers this based on the naming convention and the association between entities.
     */

    // filter these credit statements by credit statement id (or account number), credit statement month, year and type
    @Query("SELECT s FROM CreditStatement s JOIN s.creditAccount c WHERE c.customerId = ?1 ORDER BY s.creditStatementDate")
    Slice<CreditStatement> findAllByCustomerIdOrderByCreditStatementDate(String customerId, Pageable pageable);

    //

    /*
     * view all credit statements by customerId and credit account's id ordered by creditStatementDate
     *
      SELECT s.credit_statement_id, s.credit_statement_datetime, s.due_datetime, s.amount_due, s.minimum_payment,
      s.credit_account_id, c.customer_id
      FROM credit_statements s
    ...
    @Query("SELECT s FROM CreditStatement s JOIN s.creditAccount c WHERE c.customerId = ?1 AND c.id = ?2 ORDER BY s.creditStatementDate")
    Slice<CreditStatement> findAllByCustomerIdAndCreditAccount_IdOrderByCreditStatementDate(String customerId,
                                                                                       UUID creditAccountId,
                                                                                       Pageable pageable);

    Optional<CreditStatement> findByCreditAccount_IdAndCreditStatementDate(UUID creditAccountId, Date creditStatementDate);

    // save a new credit statement
    // CreditStatement save(CreditStatement creditStatement);

    interface Specs {

        static Specification<CreditStatement> byCustomerId(String customerId) {
            return (root, query, builder) ->
                    builder.equal(root.get(CreditStatement_.creditAccount)
                            .get(CreditAccount_.customerId), customerId);
        }

        static Specification<CreditStatement> byCreditAccountId(String creditAccountId) {
            return (root, query, builder) ->
                    builder.equal(root.get(CreditStatement_.creditAccount)
                            .get(CreditAccount_.id), UUID.fromString(creditAccountId));
        }

        static Specification<CreditStatement> byAccountNumber(String accountNumber) {
            return (root, query, builder) ->
                    builder.equal(root.get(CreditStatement_.creditAccount)
                            .get(CreditAccount_.accountNumber), accountNumber);
        }

        static Specification<CreditStatement> byMonth(String month) {
            return (root, query, builder) ->
                    builder.equal(
                            builder.function(
                                    "MONTH",
                                    Integer.class,
                                    root.get(CreditStatement_.creditStatementDate)
                            ),
                            Integer.parseInt(month)
                    );
        }

        static Specification<CreditStatement> byYear(String year) {
            return (root, query, builder) ->
                    builder.equal(
                            builder.function(
                                    "YEAR",
        ...
        static Specification<CreditStatement> orderByCreditStatementDate(Specification<CreditStatement> spec) {
            return (root, query, builder) -> {
                query.orderBy(
                        builder.asc(root.get(CreditStatement_.creditStatementDate))
                );
...
*
* The underscore convention in method names like findAllByCustomerIdAndCreditAccount_IdOrderByCreditStatementDateTime is
